 ...
 * The callback accesses a specific attestation provider of the Microsoft Azure Attestation (ITA)
...
static int ita_verify_response_output_quote(struct ita_response* response, const char* set_of_jwks,
    ...
    /*
     * FIXME: the self-signed token-signing certificate obtained from the `certs/` ITA API endpoint
     ...
     *
     * Unfortunately, currently ITA has two problems: (a) not all ita providers run inside the SGX
     * environment, and (b) SGX measurements of ITA are not published online but must be requested
