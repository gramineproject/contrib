...
.. warning::
   ...
   as the core Gramine documentation. Please be careful, especially in terms of
   security!
...
TensorFlow Serving running inside an SGX enclave with Gramine and its
...
- Gramine. Follow `Quick Start <https://gramine.readthedocs.io/en/latest/quickstart.html>`__
  to build Gramine. In this tutorial, we will need to build Gramine in the
...
- TensorFlow Serving cluster scripts package. You can download the scripts package
  ``tensorflow-serving-cluster`` `here <https://github.com/gramineproject/contrib>`__.

Executing TF Serving in Docker
------------------------------

We start with TensorFlow Serving running in Docker directly without Gramine.
...
Executing TF Serving without Gramine in Docker
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
scripts and go to the directory::
   ...
   cd ./contrib/tensorflow-serving-cluster/tensorflow-serving
...
Download and convert model file::
...
with our provided scripts and go to the directory::
   ...
   git clone https://github.com/RodgerZhu/contrib.git
   cd ./contrib/tensorflow-serving-cluster/tensorflow-serving
...
In later sections, we will run TensorFlow Serving with Gramine inside
SGX enclaves.

Executing TF Serving with Gramine in SGX Enclave in Docker
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In this section, we will learn how to use Gramine to run the TensorFlow Serving
...
We use ``build_gramine_tf_serving.sh`` to create Docker image with Gramine.

First, we want to highlight some options:

In ``tensorflow_model_server.manifest.template``, the manifest keys
starting with ``sgx.`` are SGX-specific syntax; these entries are ignored if
Gramine runs in non-SGX mode.

Below, we will highlight some of the SGX-specific manifest and TensorFlow Serving
options in the template.
Please refer to `this <https://gramine.readthedocs.io/en/latest/manifest-syntax.html>`__
for further details about the syntax of Gramine manifests.

We mount the entire ``<gramine repository>/Runtime/`` host-level directory to
the ``/lib`` directory seen inside Gramine. This trick allows to transparently
replace standard C libraries with Gramine-patched libraries::

   fs.mount.lib.type = "chroot"
   fs.mount.lib.path = "/lib"
   fs.mount.lib.uri  = "file:$(GRAMINEDIR)/Runtime/"
...
For SGX-specific lines in the manifest template::
   ...
   sgx.trusted_files.ld   = "file:$(GRAMINEDIR)/Runtime/ld-linux-x86-64.so.2"
   sgx.trusted_files.libc = "file:$(GRAMINEDIR)/Runtime/libc.so.6"
   ...
...
The way these Trusted Files work is before Gramine runs TensorFlow Serving inside
the SGX enclave, Gramine generates the final SGX manifest file using ``gramine-sgx-
sign`` Gramine utility. This utility calculates hashes of each trusted file and
appends them as ``sgx.trusted_checksum.<name>`` to the final SGX manifest.
When running TensorFlow Serving with SGX, Gramine reads trusted files, finds
...
Gramine will load the model file from below path::
...
will use ``gramine-sgx`` to launch the TensorFlow Serving workload inside an SGX
enclave::

    ${WORK_BASE_PATH}/gramine-sgx tensorflow_model_server \
...
Now, we can build the Docker image with Gramine, and you can set the special tag
for your Docker image::

    cd <contrib repository>/tensorflow-serving-cluster/tensorflow-serving/docker
    cp tensorflow_model_server.manifest.nonattestation.template tensorflow_model_server.manifest.template
    tag=latest
    ./build_gramine_tf_serving_image.sh ${tag}
...
Start TensorFlow Serving in untrusted machine B::
   ...
   cd <contrib repository>/tensorflow-serving-cluster/tensorflow-serving
...
Executing Kubernetes to manage TF Serving with Gramine in Docker
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
if not, please update it::
   ...
   cd <contrib repository>/tensorflow-serving-cluster/kubernetes
...
TensorFlow Serving under the directory::
   ...
   <gramine-contrib repository>/tensorflow-serving-cluster/tensorflow-serving/kubernetes
...
the Docker image created with your own tag::
    ...
    - name: model-path
      hostPath:
          path: <Your gramine repository>/Examples/tensorflow-serving-cluster/tensorflow_serving/models /resnet50-v15-fp32
    - name: ssl-path
      hostPath:
          path: <Your gramine repository>/Examples/tensorflow-serving-cluster/tensorflow_serving/ ssl_configure/ssl.cfg
...
Executing Kubernetes to manage TF Serving with Gramine with remote attestation in Docker
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

First of all, please refer to

- `Background on Remote Attestation <https://gramine.readthedocs.io/en/latest/attestation.html>`__
- `Background on Protected Files <https://gramine.readthedocs.io/en/latest/tutorials/pytorch/index.html#background-on-protected-files>`__
- `Confidential PyTorch Example <https://gramine.readthedocs.io/en/latest/tutorials/pytorch/index.html#preparing-confidential-pytorch-example>`__
...
Under the untrusted machine B, in the host, please download Gramine source code.
For simplicity, we re-use the already-existing stuff from the ``CI-Examples/ra-tls-secret-prov``
directory. In particular, we re-use the confidential wrap key::

   cd <gramine repository> && meson setup build/ --buildtype=debug -Dsgx=enabled -Ddcap=enabled -Dsgx_driver="dcap1.10" -Dsgx_driver_include_path="/gramine/driver/driver/linux/include"
   && ninja -C build/ \
   && ninja -C build/ install
   make dcap

The first line will build and install gramine, meanwhile, with ``-Ddcap=enabled``,
it will also build ``gramine/Pal/src/host/Linux-SGX/tools/`` utilities such as ``gramine-sgx-pf-crypt``
to encrypt input files.

Recall that we have the already converted model file under::

   <contrib repository>/tensorflow-serving-cluster/tensorflow-serving/models/resnet50-v15-fp32/1/saved_model.pb
...
the wrap key::
   ...
   cp <contrib repository>/tensorflow-serving-cluster/tensorflow-serving/models/resnet50-v15-fp32/1/saved_model.pb plaintext/
   gramine-sgx-pf-crypt encrypt -w files/wrap-key -i plaintext/saved_model.pb -o  models/resnet50-v15-fp32/1/saved_model.pb

We now get the encrypted model file under::

   <gramine repository>/CI-Examples/ra-tls-secret-prov/models/resnet50-v15-fp32/1/saved_model.pb
...
See `Secret Provisioning Minimal Examples <https://github.com/gramineproject/gramine/tree/master/CI-Examples/ra-tls-secret-prov>`__
for more information.

We also need to copy the server-identifying certificates so that in-Gramine
secret provisioning library can verify the provisioning server (via classical
X.509 PKI). This step is done in ``gramine_tf_serving.dockerfile`` as below::
...
Go to the directory::
   ...
   <gramine-contrib repository>/tensorflow-serving-cluster/tensorflow-serving/docker
...
Serving and Gramine add-ons search for libraries in the current directory::
...
`here <https://gramine.readthedocs.io/en/latest/attestation.html#high-level-secret-provisioning-interface>`__::
...
Build Docker image::
   ...
   tag=latest
   ./build_gramine_tf_serving.sh ${tag}

Stop any previous Kubernetes service if you started it::

   cd <gramine-contrib repository>/tensorflow-serving-cluster/kubernetes
